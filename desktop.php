...
$sql1 = "INSERT into desktop(name,phonenumber,address,email,freetime,dname,dtype,sversion,problem,pdescription) 
...
if ($insert)
{
 Header("Location: submitdesktop.php");
}     
...
<div class="title fadeInLeftBig">
          ...
          <h1><i class = "fa fa-desktop"></i>  Desktops</h1>
</div>
</div>
...
<br>
  <fieldset>
    <h3>Device Name</h3><input placeholder="Device Name" type="text" tabindex="5" name="dname" required >
     ...
     <h3>Device Type</h3>   
    <fieldset>
    <input placeholder="Enter the Device Type(i.e tower case)" type="text" tabindex="6" name="dtype"  >
    </fieldset>
  ...
  <option value="deadscreen">Dead monitor screen</option>
  <option value="uresponsivescreen">unresponsive screen</option>
  ...
  <option value="spoiledcamerafront">Spoiled webcam camera</option>
  <option value="spoiledcameraback">Blue screen</option>
  <option value="unresponsivekeys">unresponsive keys</option>
  <option value="notcharging">input/output problems</option>
